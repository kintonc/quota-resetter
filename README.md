...
- After a successful Claude or Antigravity kick, the next five-hour reset is scheduled automatically. A newer timestamp from the service's status-line cache replaces that estimate when available; a stale pre-kick snapshot is ignored.
- An expired Codex reset remains pending through the grace period and failed attempts, even if the live API starts reporting a newer sliding reset. One-second API timestamp jitter is ignored.
